...
Процедура СоздатьРеквизиты(Форма, СтруктураРеквизитов) Экспорт
	...
	Для Каждого тРеквизит Из СтруктураРеквизитов Цикл
		...
		//++dar 25.02.26 (309302)
		//Если НЕ ОбщегоНазначения.ЕстьРеквизитОбъекта(Форма, тРеквизит.Ключ) Тогда
		Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма,тРеквизит.Ключ) Тогда
		//--dar 25.02.26 (309302)
			ДобавляемыеРеквизиты.Добавить( Новый РеквизитФормы(тРеквизит.Ключ,тРеквизит.Значение) );
		КонецЕсли;
...
//  Картинка - Картинка - Изображение, которое будет установлено команде
//
Функция НоваяКоманда(Форма,ИмяКоманды,Заголовок,ИмяДействия, Картинка = Неопределено) Экспорт
...
Функция СоздатьЭлементФормы(Форма,ИмяЭлемента,ТипЭлемента,РодительЭлемента,ЭлементПеред = Неопределено)	
	...
	НовыйЭлемент = Форма.Элементы.Найти(ИмяЭлемента);
	...
	ИначеЕсли ЭлементПеред = Неопределено Тогда
		Форма.Элементы.Переместить(НовыйЭлемент, РодительЭлемента, ЭлементПеред);	
	КонецЕсли;
...
Процедура СоздатьКолонкиДляТаблицыФормы(Форма,ТаблицаФормы,СтруктураКолонок,ЭтоДС = ЛОЖЬ)
	...
	Для каждого тКолонка из СтруктураКолонок Цикл
		...
		НоваяКолонкаТаблицы = СоздатьЭлементФормы(Форма, ИмяСписка + "_" + ИмяКолонки, Тип("ПолеФормы"), ТаблицаФормы);
		//--dar 25.07.24 (173039)
		...
		Если НЕ ЗначениеЗаполнено(НоваяКолонкаТаблицы.ПутьКДанным) Тогда
			//++dar 25.07.24 (173039)
			//НоваяКолонкаТаблицы.ПутьКДанным = ИмяСписка + "." + ИмяКолонки;
			НоваяКолонкаТаблицы.ПутьКДанным = ПутьКДанным + "." + ИмяКолонки;
			//--dar 25.07.24 (173039)
		КонецЕсли;
...
//
Функция НоваяГруппаОбычнаяБезОтображения(Форма,
	ИмяГруппы,
	РодительГруппы,
...
//					- Элементы - можно передать элемент формы
//
Функция НоваяГруппаОбычная(Форма,
	ИмяГруппы,
	РодительГруппы,
	Заголовок = "",
	ГруппировкаОбычнойГруппы = 0,
...
//					- Элементы - можно передать элемент формы
//
Функция НоваяГруппаСтраниц(Форма,
	ИмяГруппы,
	РодительГруппы,
	РастягиватьГоризонтально = 0,
	РастягиватьВертикально = 0,
...
//					- Элементы - можно передать элемент формы
//
Функция НоваяГруппаКоманднаяПанель(Форма,
	ИмяПанели,
	РодительПанели,
	СтруктураСвойств = Неопределено,
	ЭлементПеред = Неопределено) Экспорт
...
//					- Булево - Если Ложь то ПутьКДанным равен параметру ИмяПоля. Если Истина то ПутьКДанным = Объект[ИмяПоля]
...
//
Функция НовоеПоле(Форма, ИмяПоля, РодительПоля, Знач ПутьКДанным = "", ШифрВидаПоля = 1, ЭлементПеред = Неопределено, Заголовок = "", 
	СтруктураСвойств = Неопределено, СтруктураСобытий = Неопределено) Экспорт
	...
	Если ТипЗнч(ПутьКДанным) = Тип("Булево") Тогда
		пПутьКДанным = ?(ПутьКДанным, "Объект.", "") + ИмяПоля;
	Иначе
		пПутьКДанным = ПутьКДанным;
	КонецЕсли;
	Если пПутьКДанным <> "" и НовыйЭлемент.ПутьКДанным <> пПутьКДанным Тогда
		НовыйЭлемент.ПутьКДанным = пПутьКДанным;
	КонецЕсли;
...
//					- Элементы - можно в качестве родителя передать элемент формы
//	ПутьКДанным		- Строка - Путь к данным значения
//					- Булево - Если Ложь то ПутьКДанным равен параметру ИмяПоля. Если Истина то ПутьКДаннымм = Объект[ИмяПоля]
//	Заголовок		- Строка - заголовок элемента
...
Функция НовоеПолеФлажок(Форма,
	...
	НовыйЭлемент = НовоеПоле(Форма, ИмяПоля, РодительПоля, ПутьКДанным, 2, ЭлементПеред, Заголовок, СтруктураСвойств, СтруктураСобытий);
...
//					- Элементы - можно передать элемент формы
//
Функция НоваяДекорацияНадпись(Форма,
	ИмяДекорации,
	РодительДекорации,
	ЗаголовокДекорации,
	...
	НовыйЭлемент = НоваяДекорация(Форма,ИмяДекорации,РодительДекорации,,,ЭлементПеред);
...
//++dar 22.06.26 (333905)
// Прижимает кнопку отрицательного выполнения ("Не согласовано") к правому краю
// горизонтальной группы команд выполнения задачи.
//
// Параметры:
//  Форма  - ФормаКлиентскогоПриложения
//  Группа - ГруппаФормы - горизонтальная группа с кнопками выполнения (ГруппаКомандыДляВыполнения)
//
// Прижимает кнопку "Не согласовано" к правому краю строки кнопок выполнения задачи.
//
// Параметры:
//  Форма  - ФормаКлиентскогоПриложения
//  Группа - ГруппаФормы - группа кнопок выполнения (ГруппаКомандыДляВыполнения)
//
Процедура ПрижатьКнопкуНеСогласованоВправо(Форма, Группа) Экспорт
	
	Если Группа = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	КнопкаНеСогласовано = Неопределено;
	Для Каждого Элемент Из Группа.ПодчиненныеЭлементы Цикл
		Если ТипЗнч(Элемент) = Тип("КнопкаФормы")
			И Элемент.Заголовок = НСтр("ru = 'Не согласовано'") Тогда
			КнопкаНеСогласовано = Элемент;
		КонецЕсли;
	КонецЦикла;
	
	Если КнопкаНеСогласовано = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Группа.РастягиватьПоГоризонтали = Истина;
	
	// невидимая кнопка-распорка
	ИмяРаспорки = "УВМ_КнопкаРаспорка_" + Группа.Имя;
	Распорка = Форма.Элементы.Найти(ИмяРаспорки);
	Если Распорка = Неопределено Тогда
		ИмяКоманды = "УВМ_КомандаРаспорка";
		Если Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
			Команда = Форма.Команды.Добавить(ИмяКоманды);
			Команда.Действие = "УВМ_РаспоркаНоп";
			Команда.Заголовок = " ";
		КонецЕсли;
		Распорка = Форма.Элементы.Добавить(ИмяРаспорки, Тип("КнопкаФормы"), Группа);
		Распорка.ИмяКоманды = ИмяКоманды;
	КонецЕсли;
	Распорка.Заголовок = " ";
	Распорка.Доступность = Ложь;
	Распорка.ОтображениеФигуры = ОтображениеФигурыКнопки.Нет;
	Распорка.АвтоМаксимальнаяШирина = Ложь;
	Распорка.РастягиватьПоГоризонтали = Истина;
	
	// распорку — прямо перед "Не согласовано": она оттолкнёт кнопку к правому краю
	Форма.Элементы.Переместить(Распорка, Группа, КнопкаНеСогласовано);
...
//					- Элементы - можно передать элемент формы
//
Функция НоваяДекорацияКартинка(Форма,
	ИмяДекорации,
	РодительДекорации,
	ИмяКартинки,
	ЭлементПеред = Неопределено) Экспорт
...
//					- Элементы - можно передать элемент формы
//
Функция НовыйДинамическийСписок(Форма,
	ИмяСписка = "Список", 
	РодительСписка,
	ТекстЗапроса = "", 			
	...
	ТаблицаФормы = НоваяТаблицаФормы(Форма,ИмяСписка,РодительСписка,ИмяСписка,СтруктураКолонок,СтруктураСвойств,СтруктураСобытий,СвояКоманднаяПанель,ЭлементПеред,Истина); 
...
//					- Элементы - можно передать элемент формы
//
Функция НоваяКолонкаТаблицыФормы(Форма, Знач ИмяКолонки, Родитель, Знач ПутьКДанным, Знач ТипКолонки = 0, Знач Заголовок = "", Знач ОтображатьШапку = Истина, 
	Знач ОтображатьПодвал = Ложь, Знач СтруктураСвойств = Неопределено, Знач СтруктураСобытий = Неопределено, ЭлементПеред = Неопределено) Экспорт
...
//					- Элементы - можно передать элемент формы
//
Функция НоваяТаблицаФормы(Форма, Знач ИмяТаблицы, РодительТаблицы, Знач ПутьКДанным, Знач СтруктураКолонок = Неопределено, Знач СтруктураСвойств = Неопределено,
	Знач СтруктураСобытий = Неопределено, Знач СвояКоманднаяПанель = Ложь, ЭлементПеред = Неопределено, Знач ЭтоДС = Ложь) Экспорт	
	...
	Если СвояКоманднаяПанель Тогда
		ГруппаДинамическогоСписка = НоваяГруппаОбычнаяБезОтображения(Форма,"ДополнительнаяГруппаКоманднаяПанель" + ИмяТаблицы,РодительТаблицы,ЭлементПеред,1,1,1);
		КоманднаяПанельСписка = НоваяГруппаКоманднаяПанель(Форма,ИмяТаблицы + "КоманднаяПанель2",ГруппаДинамическогоСписка);
	...
	Если НЕ ЗначениеЗаполнено(ТаблицаФормы.ПутьКДанным) Тогда
		
		Если ТипЗнч(ПутьКДанным) = Тип("Булево") Тогда
			ТаблицаФормы.ПутьКДанным = ?(ПутьКДанным, "Объект.", "") + ИмяТаблицы;	
		Иначе
	...
	СоздатьКолонкиДляТаблицыФормы(Форма, ТаблицаФормы, СтруктураКолонок, ЭтоДС);
...
//					- Элементы - можно передать элемент формы
//
Функция НоваяГруппаКолонокТаблицыФормы(Форма,
	ИмяГруппы,
	РодительГруппы,
	ГруппировкаГруппы = 0,
	РастягиватьГоризонтально = 0,
...
//					- Элементы - можно передать элемент формы
//
Функция НовыйВыпадающийСписок(Форма,
	ИмяСписка,
	РодительСписка,
	ЗаголовокЭлемента = "",
	МассивЗначений,
	...
	НовыйЭлемент = НовоеПоле(Форма,ИмяСписка,РодительСписка,ИмяСписка,1,ЭлементПеред,ЗаголовокЭлемента,СтруктураСвойств,СтруктураСобытий);
...
//					- Элементы - можно передать элемент формы
//
Функция НовыйПереключатель(Форма,
	ИмяПереключателя,
	РодительЭлемента,
	ЗаголовокЭлемента = "",
	МассивЗначений,
	...
	НовыйЭлемент = НовоеПоле(Форма,ИмяПереключателя,РодительЭлемента,ИмяПереключателя,4,ЭлементПеред,ЗаголовокЭлемента,СтруктураСвойств,СтруктураСобытий);
	//НовыйЭлемент.ВидПоля = ВидПоляФормы.ПолеПереключателя; //++РомановНЕ 20230906 - закоментировал, потому что падает в ошибку, в строке выше уже и так указываем вид поля
...
#Область КомбинированныеФункции 

Функция НоваяКомандаНаФорме(Форма, ИмяКоманды, РодительКнопки, Заголовок = "", Картинка = Неопределено, 
		ВидКнопки = 0, СтруктураСвойств = Неопределено,	ЭлементПеред = Неопределено) Экспорт

	Команда = НоваяКоманда(Форма, ИмяКоманды, Заголовок, ИмяКоманды, Картинка);
	Кнопка = НоваяКнопка(Форма, ИмяКоманды, РодительКнопки, Заголовок, ИмяКоманды, ВидКнопки, СтруктураСвойств, ЭлементПеред);
...
#Область ПроцедурыИФункцииДляРаботыСДополнительнымиРеквизитами
...
#Область ПараметрыВыбораИСвязи

Процедура УстановитьПараметрыВыбораЭлементаФормы(вхЭлементФормы,вхМассивДанныхПараметровВыбора) Экспорт	
	тПараметрыВыбора 	= Новый Массив();	
	Для Каждого тДанныеПараметровВыбора из вхМассивДанныхПараметровВыбора Цикл
		Если ТипЗнч(тДанныеПараметровВыбора) = Тип("Структура") Тогда
			тПараметр = Новый ПараметрВыбора(тДанныеПараметровВыбора.Имя, тДанныеПараметровВыбора.Значение);			
		ИначеЕсли ТипЗнч(тДанныеПараметровВыбора) = Тип("ПараметрВыбора") Тогда
			тПараметр = тДанныеПараметровВыбора;
		КонецЕсли;
		тПараметрыВыбора.Добавить(тПараметр);	
	КонецЦикла;
	вхЭлементФормы.ПараметрыВыбора = Новый ФиксированныйМассив(тПараметрыВыбора);
КонецПроцедуры

Процедура УстановитьСвязиПараметровВыбораЭлементаФормы(вхЭлементФормы,вхМассивДанныхСвязейПараметровВыбора) Экспорт
	тМассивСвязей 	= Новый Массив();
	Для Каждого тДанныеСвязейПараметровВыбора из вхМассивДанныхСвязейПараметровВыбора Цикл
		Если ТипЗнч(тДанныеСвязейПараметровВыбора) = Тип("Структура") Тогда                                            
			тИзменениеЗначение = РежимИзмененияСвязанногоЗначения.Очищать;
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(тДанныеСвязейПараметровВыбора,"ИзменениеЗначения") Тогда
				тИзменениеЗначение = тДанныеСвязейПараметровВыбора.ИзменениеЗначения;
			КонецЕсли;
			тСвязь = Новый СвязьПараметраВыбора(тДанныеСвязейПараметровВыбора.Имя, тДанныеСвязейПараметровВыбора.ПутьКДанным,тИзменениеЗначение);			
		ИначеЕсли ТипЗнч(тДанныеСвязейПараметровВыбора) = Тип("СвязьПараметраВыбора") Тогда
			тСвязь = тДанныеСвязейПараметровВыбора;
		КонецЕсли;
		тМассивСвязей.Добавить(тСвязь);	
	КонецЦикла;
	вхЭлементФормы.СвязиПараметровВыбора = Новый ФиксированныйМассив(тМассивСвязей); 
КонецПроцедуры
