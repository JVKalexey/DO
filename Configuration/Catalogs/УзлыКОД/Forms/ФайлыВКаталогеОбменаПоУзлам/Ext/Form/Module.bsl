...
Процедура ЗаполнитьФайлы(ТекУзелКОД, ТаблЗнач)
	...
	КодЭтогоУзла = КОДПовтИсп.КодТекущегоУзла();
	//@skip-check bsl-legacy-check-string-literal
	КодТекУзла = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекУзелКОД, "Код");
	...
	Если ЗначениеЗаполнено(ОбщийКаталог) Тогда
		...
		Если Каталог.Существует() Тогда
			...
			МаскаФайлов = СтрШаблон("%1to%2at*.json", КодТекУзла, КодЭтогоУзла);
			...
			Если МассивФайлов.Количество() = 0 Тогда
				...
				Для Каждого ФайлИлиКаталог Из ФайлыИКаталоги Цикл
					Если ФайлИлиКаталог.Существует()
							И ФайлИлиКаталог.ЭтоКаталог()
			...
			МаскаФайлов = СтрШаблон("%1to%2at*.json", КодЭтогоУзла, КодТекУзла);
			МассивФайлов = НайтиФайлы(ОбщийКаталог, МаскаФайлов, Ложь);
			Если МассивФайлов.Количество() = 0 Тогда
				ФайлыИКаталоги = НайтиФайлы(ОбщийКаталог, "*.*", Ложь);
				Для Каждого ФайлИлиКаталог Из ФайлыИКаталоги Цикл
