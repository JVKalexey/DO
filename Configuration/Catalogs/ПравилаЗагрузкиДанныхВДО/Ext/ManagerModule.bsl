...
// Рекурсивный вызов для обхода всех вышестоящих родителей правила обработки.
//
// Параметры:
//   ПравилоОбработкиСРодителями - Массив из СправочникСсылка.ПравилаОбработки - контейнер, куда добавляется
//     правило обработки и родители всех уровней.
//   ПравилоОбработки - СправочникСсылка.ПравилаОбработки - правило обработки текущего уровня.
//
Процедура ДобавитьРодителейПравилаОбработки(ПравилоОбработкиСРодителями, ПравилоОбработки)
	
	РодительПравила = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПравилоОбработки, "Родитель");
	Если ЗначениеЗаполнено(РодительПравила) Тогда
		ПравилоОбработкиСРодителями.Добавить(РодительПравила);
		ДобавитьРодителейПравилаОбработки(ПравилоОбработкиСРодителями, РодительПравила);
	КонецЕсли;
...
Процедура ДополнитьРеквизитыДокумента(Реквизиты, ВидДокументаОбъект, Настройки)
	...
	НастройкиИВидыДействий = НастройкиИВидыДействийПравилаОбработки(ВидДокументаОбъект.Ссылка);
	Для Каждого ВидДействия Из НастройкиИВидыДействий.ДанныеВидовДействий Цикл
		...
		Для Каждого СтрокаСвязи Из НастройкиИВидыДействий.СвязьНастроекИВидовДействий.НайтиСтроки(Отбор) Цикл
			...
			Если НастройкаСЭтапами <> Неопределено Тогда
				...
				ПравилоОбработкиСРодителями.Добавить(СтрокаСвязи.ПравилоОбработки);
				ДобавитьРодителейПравилаОбработки(ПравилоОбработкиСРодителями, СтрокаСвязи.ПравилоОбработки);
				ПредставлениеОбработки = ПредставлениеОбработки(ПравилоОбработкиСРодителями);
		...
		ДопОбработчик.ИмяМетодаПроверкиДоступностиПоСостоянию = "ПроверитьДоступностьПоСостоянию_ДействиеОбработки";
...
Функция НастройкиИВидыДействийПравилаОбработки(ВидОбъектаОбработки)
	...
	ПравилаОбработки = ПравилаОбработкиСервер.ПравилаОбработкиПоВидуОбъекта(ВидОбъектаОбработки, Ложь, Истина);
	Для Каждого Строка Из ПравилаОбработки Цикл
		НастройкиИВидыДействийДляПравила = РегистрыСведений.ПравилаОбработкиОбъектов.НастройкиИВидыДействийДляПравила(
			Строка.ПравилоОбработки, ПравилаОбработки[0]);
		Для Каждого СтрокаНастройки Из НастройкиИВидыДействийДляПравила Цикл
			Если СтрокаНастройки.НастройкаВключена Тогда
				...
				Если ТипНастройки = Тип("СправочникСсылка.НастройкиДействийСогласования")
						Или ТипНастройки = Тип("СправочникСсылка.НастройкиДействийПодписания") Тогда
					Если НастройкиСЭтапами.Найти(СтрокаНастройки.Настройка) = Неопределено Тогда
						НастройкиСЭтапами.Добавить(СтрокаНастройки.Настройка);
					КонецЕсли;
				ИначеЕсли ТипНастройки = Тип("СправочникСсылка.НастройкиДействийИсполнения") Тогда
					Если НастройкиИсполнения.Найти(СтрокаНастройки.Настройка) = Неопределено Тогда
	...
	Результат.Вставить("ДанныеВидовДействий", ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ВидыДействий, "Тип, Наименование"));
	Результат.Вставить("ДанныеНастроекСЭтапами", ОбщегоНазначения.ЗначенияРеквизитовОбъектов(НастройкиСЭтапами, "Этапы"));
	Результат.Вставить("ДанныеНастроекИсполнения", ОбщегоНазначения.ЗначенияРеквизитовОбъектов(НастройкиИсполнения, "ПоПунктам"));
...
Функция ПредставлениеОбработки(ПравилоОбработкиСРодителями)
