...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Локализация
	СтатусРаспознаванияДокумента = РаспознаваниеДокументов.ТекущаяИнформацияОСтатусеРаспознаванияДокумента(Объект.Ссылка, Истина, Истина); 
	// Конец Локализация
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры.Основание) Тогда // Ввод на основании
		...
		ИначеЕсли ТипЗнч(ОснованиеДокумента) = Тип("СправочникСсылка.Мероприятия")
			Или ТипЗнч(ОснованиеДокумента) = Тип("ДокументСсылка.ВходящееПисьмо")
			Или ТипЗнч(ОснованиеДокумента) = Тип("ДокументСсылка.ИсходящееПисьмо") Тогда
    ...
    // Локализация
	РаспознаваниеДокументовСлужебный.ЗаполнитьВерифицируемыеРеквизитыПриОткрытииФормыДокумента(Объект.Ссылка, ВерифицируемыеРеквизиты);	
	// Конец Локализация    
	...
	ДокументыПредприятияЛокализация.ПриСозданииНаСервереФормаЭлемента(ЭтотОбъект, Отказ, 
																	СтандартнаяОбработка, Параметры);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменениеВерифицируемыхРеквизитов" и Параметр.Предмет = Объект.Ссылка Тогда
		
		Модифицированность = Истина;
		ВсеРеквизиты = Параметр.Свойство("ВсеРеквизиты");
		
		Для Каждого Стр Из Параметр.ВерифицированныеРеквизиты Цикл
			...
			НСтр = ВерифицируемыеРеквизиты.НайтиСтроки(Отбор);
			
			Если НСтр.Количество() > 0 Тогда
				
				ВерифицированныйРеквизит = НСтр[0]; //укажем что верифицирован
				ВерифицированныйРеквизит.Проверен = Истина;
				ВерифицированныйРеквизит.ТекущееЗначение = Стр.ТекущееЗначение;
				
				Если ВсеРеквизиты = Истина Тогда
					Прервать;
				КонецЕсли;
				
				Если Не ПустаяСтрока(ВерифицированныйРеквизит.ТабличнаяЧасть) Тогда //заполним текущее значение  
					
					ТЧ = Вычислить(ВерифицированныйРеквизит.ТабличнаяЧасть);
					ТЧ[ВерифицированныйРеквизит.НомерСтрокиТЧ - 1][ВерифицированныйРеквизит.ИмяРеквизита] = ВерифицированныйРеквизит.ТекущееЗначение;
					
				Иначе
					
					Если СтрНайти(ВерифицированныйРеквизит.ИмяРеквизита, "Объект.") > 0 Тогда
						Объект[СтрЗаменить(ВерифицированныйРеквизит.ИмяРеквизита, "Объект.", "")] = ВерифицированныйРеквизит.ТекущееЗначение;
					Иначе
						ЭтотОбъект[ВерифицированныйРеквизит.ИмяРеквизита] = ВерифицированныйРеквизит.ТекущееЗначение;
					КонецЕсли;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ПереносКэшаИзЧтенияВСоздание =
		КэшиНаВремяВызова.ИнициализироватьДокументыПредприятияФормаЭлемента(ТекущийОбъект);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ТипЗнч(Основание) = Тип("СправочникСсылка.ДокументыПредприятия") 
		И Не ТипСвязиВыбран 
		И Не ОкноВыбораТипаСвязиПоказано 
	...
	Если ЭтоСозданиеНаОснованииДокумента И Не ЭтоНагрузочноеТестирование
		И Не ПоказанМастерСозданияНаОсновании Тогда
		
		НадоПоказатьМастер = Ложь;  
		
		Если ВидДокументаКэш.УчитыватьНедействующиеДокументы
			Или (ОбязательныеТипыСвязей.Количество() <> 0 И
			ОбязательныеТипыСвязей.Количество() <> ОбязательныеСвязи.Количество())
			Или (Не ТипСвязиВыбран И Не ОкноВыбораТипаСвязиПоказано И МассивТиповСвязей.Количество() > 0)
			Тогда
		...
		Если НадоПоказатьМастер Тогда 
			...
			ОписаниеОповещения = Новый ОписаниеОповещения(
				"ПередЗаписьюПродолжениеПослеМастерСозданияНаОсновании",
				ЭтотОбъект,
				ПараметрыЗаписи);
			...
			ПараметрыФормы.Вставить("ПоказатьОбязательныеСвязи", (ОбязательныеТипыСвязей.Количество() <> 0 И
				ОбязательныеТипыСвязей.Количество() <> ОбязательныеСвязи.Количество()));
			...
			ОткрытьФорму(
				"Справочник.ДокументыПредприятия.Форма.МастерСозданияНаОсновании",
				ПараметрыФормы,,,,,
				ОписаниеОповещения);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	// Локализация 
	ТекущийОбъект.ДополнительныеСвойства.Вставить("ДокументОтправленНаРаспознавание", СтатусРаспознаванияДокумента = 
	...
	ДокументыПредприятияЛокализация.ПередЗаписьюНаСервере_ФормаЭлемента(ЭтотОбъект, Отказ, ТекущийОбъект, ПараметрыЗаписи);
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	// Локализация
	РаспознаваниеДокументовСлужебный.УстановкаВерификацииПриЗаписиДокумента(Объект.Ссылка, ВерифицируемыеРеквизиты, Отказ);
	// Конец Локализация
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(ТекущийФайлВСпискеДобавленныхДляРедактирования) Тогда   
		ВладелецФайла = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийФайлВСпискеДобавленныхДляРедактирования, "ВладелецФайла");
...
Процедура СуммаПриИзменении(Элемент)
	...
	// Локализация  
	ПриИзмененииВерифицируемогоРеквизита("Объект.Сумма", Объект.Сумма);
	// Конец Локализация
...
Процедура СуммаНДСПриИзменении(Элемент)
	...
	// Локализация
	ПриИзмененииВерифицируемогоРеквизита("Объект.СуммаНДС", Объект.СуммаНДС);
	// Конец Локализация
...
Процедура ВалютаПриИзменении(Элемент)
	...
	// Локализация 
	ПриИзмененииВерифицируемогоРеквизита("Объект.Валюта", Объект.Валюта);
	// Конец Локализация
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	// Локализация
	ПриИзмененииВерифицируемогоРеквизита("Объект.Контрагент", Объект.Контрагент);
	// Конец Локализация
...
Процедура РаспознаваниеДокументаНадписьНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("ВерифицируемыеРеквизиты", ПолучитьРеквизитыДляВерификации(ВерифицируемыеРеквизиты));
	
	ОткрытьФорму("Справочник.ДокументыПредприятия.Форма.РаспознаваниеДокумента", ПараметрыФормы, ЭтаФорма);
...
Процедура КонтрагентыКонтрагентПриИзменении(Элемент)
	...
	// Локализация     
	ПриИзмененииВерифицируемогоРеквизитаТЧ("Контрагент", "Объект.Контрагенты", ТекущиеДанные);  
	// Конец Локализация
...
Процедура ФайлыПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные <> Неопределено
		...
		Если ТекущиеДанные.Ссылка <> ТекущийФайл Или ДокументыПредприятияКлиентСерверЛокализация.СтрокаФайловНеЯвляетсяФайлом(ТекущиеДанные) Тогда
			ТекущийФайл = ТекущиеДанные.Ссылка;
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	// Локализация
	ПриИзмененииВерифицируемогоРеквизитаТЧ("Номенклатура", "Объект.Товары", ТекущиеДанные); 
	// Конец Локализация
...
Процедура СохранитьКак(Команда)
	...
	Если ВыделенныеСтроки.Количество() > 1 Тогда
		...
		Для Каждого ВыбраннаяСтрока Из ВыделенныеСтроки Цикл
			...
			Если Не ДанныеСтроки.ЭтоДокумент И Не ДанныеСтроки.ЭтоРольФайла 
				И Не ДанныеСтроки.ЭтоДокументЭДО Тогда
	...
	Иначе
		...
		ДанныеФайла = РаботаСФайламиВызовСервера.ДанныеФайлаДляСохранения(ТекущиеДанные.Ссылка, Неопределено, УникальныйИдентификатор);
		КомандыРаботыСФайламиКлиент.СохранитьКак(ДанныеФайла, УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ПерейтиКВыполнениюЗадачи(Команда)  
	...
	МассивИДСтрок = ИДСтрокЗадачМне.ВыгрузитьЗначения();

	Если МассивИДСтрок.Количество() > 1 Тогда  
		
		МассивУчастников = Новый Массив;
		Для Каждого ИдСтроки Из МассивИДСтрок Цикл 
			
			СтрокаДействия = Действия.НайтиПоИдентификатору(ИдСтроки);
			Если СтрокаДействия <> Неопределено Тогда
				СтруктУчастника = Новый Структура("Действие, Идентификатор", 
					СтрокаДействия.Действие, СтрокаДействия.Идентификатор);
				МассивУчастников.Добавить(СтруктУчастника);	
			КонецЕсли;	
			
		КонецЦикла;	
		
		МассивДействийЗадач = ДействияВызовСервера.ДействияЗадачУчастниковДействияОбработки(МассивУчастников);
		
		СписокДействий = Новый СписокЗначений;
		Для Каждого СтруктДействия Из МассивДействийЗадач Цикл
			СписокДействий.Добавить(СтруктДействия.ДействиеЗадачи, СтруктДействия.Представление);
		КонецЦикла;	
		
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ОткрытьСписокВыбораЗадачиПродолжение",
			ЭтотОбъект);
		
		ПоказатьВыборИзСписка(ОписаниеОповещения, СписокДействий, Элементы.ПерейтиКВыполнениюЗадачи);
		
		
	Иначе // 1 строка	           
		
		СтрокаДействия = Действия.НайтиПоИдентификатору(МассивИДСтрок[0]);
		ОткрытьФормуЗадачи(СтрокаДействия);
...
Процедура ОткрытьРедакторТекстовыхДанныхТаймлист(КлючИсточника = "")
	...
	ТекущиеДанныеФайлов = ТекущиеДанныеСпискаФайлов();
	ОповещениеОЗакрытииФормыТаймлист = Новый ОписаниеОповещения("ПослеЗавершенияРедактированияТекстовыхДанныхТаймлист",
		ЭтотОбъект);
	ТаймлистКлиент.ОткрытьРедакторТекстовыхДанных(ЭтотОбъект, ТекущиеДанныеФайлов.Ссылка,
...
Процедура УстановитьВидимостьСтраницОбзораФайла()
	...
	ДокументыПредприятияКлиентЛокализация.УстановитьВидимостьСтраницОбзораФайлаФормаЭлемента(ЭтотОбъект);
...
Процедура УстановитьДоступностьКоманд(ТекущиеДанные)
	...
	Иначе
		...
		УстановитьДоступностьКоманды(Элементы["Занять"+Префикс], 
			Не ЗначениеЗаполнено(Редактирует) И РазрешеноРедактирование);
		...
		ДоступнаВставитьРегистрационныйНомер = 
			...
			И Не ПодписанЭП И Не ЗначениеЗаполнено(Редактирует) И Не ДокументВАрхивеИлиУничтожен;
...
Процедура ОбновитьОбластьЗадач()
	...
	Если Не Объект.Ссылка.Пустая()
		...
		Для Каждого СтрокаДействия Из Действия.ПолучитьЭлементы() Цикл
			ПроверитьЧтоЕстьЗадачаМне(СтрокаДействия, ПользовательИЕгоСотрудники, РолиТекущегоПользователя, ЗадачаНайдена, МассивИДСтрок);
		КонецЦикла;	
		...
		Иначе	
			...
			УстановитьТекстГруппаЗадач(МассивИДСтрок);
...
Процедура ПроверитьЧтоЕстьЗадачаМне(СтрокаДействия, ПользовательИЕгоСотрудники, РолиТекущегоПользователя,
	...
	Если ЭтоМояЗадача Тогда
		...
		Если СтрокаДействия.ЭтоУчастник
			И Не СтрокаДействия.ЭтоДействие 
		...
		МассивИДСтрок.Добавить(СтрокаДействия.ПолучитьИдентификатор());
		ЗадачаНайдена = Истина;
		
	КонецЕсли;
	
	Для Каждого СтрокаДействияТек Из СтрокаДействия.ПолучитьЭлементы() Цикл
		ПроверитьЧтоЕстьЗадачаМне(СтрокаДействияТек, ПользовательИЕгоСотрудники, РолиТекущегоПользователя, 
			ЗадачаНайдена, МассивИДСтрок);
	КонецЦикла;
...
&НаСервере
Процедура УстановитьТекстГруппаЗадач(МассивИДСтрок)      
	
	ИДСтрокЗадачМне.ЗагрузитьЗначения(МассивИДСтрок);
	
	Если МассивИДСтрок.Количество() > 1 Тогда     
		
		СтрокаЗадачи = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
			НСтр("ru = ';%1 задача;;%1 задачи;%1 задач;%1 задач'"),
			МассивИДСтрок.Количество());
			
		Элементы.ДекорацияЗадач.Заголовок = 
			СтрШаблон(НСтр("ru = 'У Вас %1 по документу'"), 
			СтрокаЗадачи);
...
&НаКлиенте
Процедура ОткрытьФормуЗадачи(СтрокаДействия)
...
&НаКлиенте
Функция ПолучитьРеквизитыДляВерификации(Знач ВерифицируемыеРеквизиты)
	
	МассивРеквизитов = Новый Массив;
	
	Для Каждого Стр Из ВерифицируемыеРеквизиты Цикл    
		
		Если Стр.Проверен = Истина Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОрганизацияПриИзмененииНаСервере()
	...
	// Локализация
	ПриИзмененииВерифицируемогоРеквизитаНаСервере("Объект.Организация", Объект.Организация);
	// Конец Локализация
...
// При изменении верифицируемого реквизита.
// 
// Параметры:
//  ПутьКДанным - Строка - Путь к данным
//  Значение - СправочникСсылка.Контрагенты, Число, СправочникСсылка.Валюты - Значение
&НаКлиенте
Процедура ПриИзмененииВерифицируемогоРеквизита(ПутьКДанным, Значение) 
	
	Если ВерифицируемыеРеквизиты.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ОбработатьВерифицируемыеРеквизиты(ПутьКДанным, Значение, ВерифицируемыеРеквизиты);
	УстановитьДоступностьПолей(); 
	
КонецПроцедуры   

&НаСервере
Процедура ПриИзмененииВерифицируемогоРеквизитаНаСервере(ПутьКДанным, Значение) 
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбработатьВерифицируемыеРеквизиты(ПутьКДанным, Значение, ВерифицируемыеРеквизиты) 
	...
	Отбор = Новый Структура; 
	Отбор.Вставить("ИмяРеквизита", ПутьКДанным);
	...
	Если ВР.Количество() > 0 Тогда
		Если ВР[0].Проверен = Ложь Тогда  
			ВР[0].Проверен = Истина;
			ВР[0].ТекущееЗначение = Значение;	
		КонецЕсли;
	КонецЕсли; 
...
&НаКлиенте
Процедура ПриИзмененииВерифицируемогоРеквизитаТЧ(ИмяРеквизита, ТабличнаяЧасть, ТекущиеДанные)   
	...
	Отбор.Вставить("ТабличнаяЧасть", ТабличнаяЧасть);
	Отбор.Вставить("НомерСтрокиТЧ", ТекущиеДанные.НомерСтроки);
	Отбор.Вставить("Проверен", Ложь);
	...
	Если ВР.Количество() > 0 Тогда
		Если ВР[0].Проверен = Ложь Тогда  
			ВР[0].Проверен = Истина;
			ВР[0].ТекущееЗначение = ТекущиеДанные[ИмяРеквизита];
		КонецЕсли;
	КонецЕсли;
