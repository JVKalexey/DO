...
Процедура ОтборВидДокументаПриИзменении(Элемент)
	...
	УстановитьОтборВидыДействий(Истина);
...
Процедура ЗаполнитьНастройкиДоступности()
	...
	Для Каждого Строка Из Делопроизводство.ПолучитьИменаПолейИКомандДляНастройкиДоступности() Цикл
		...
		Если Строка.ИмяПоляКоманды = "ДопРеквизиты" Тогда
			...
			ЗаполнитьДанныеДополнительныхРеквизитовИСведений(НоваяСтрока, ДопРеквизитыИСведения);
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура УстановитьОтборВидыДействий(ОбновитьДопРеквизиты = Ложь)
	...
	Если ОбновитьДопРеквизиты Тогда
		
		ДопРеквизиты = ДополнительныеРеквизитыИСведения(ОтборВидДокумента);
		
		НастройкиДоступностиДерево = РеквизитФормыВЗначение("НастройкиДоступности");
		УзелДопРеквизиты = НастройкиДоступностиДерево.Строки.Найти("ДопРеквизиты", "ИмяПоляКоманды");
		
		Если УзелДопРеквизиты <> Неопределено Тогда
			УзелДопРеквизиты.Строки.Очистить();
			ЗаполнитьДанныеДополнительныхРеквизитовИСведений(УзелДопРеквизиты, ДопРеквизиты);
			ЗначениеВРеквизитФормы(НастройкиДоступностиДерево, "НастройкиДоступности");
		КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьДанныеДополнительныхРеквизитовИСведений(УзелДопРеквизиты, ДопРеквизитыИСведения)
	
	Отбор = Новый Структура("ИмяПоляКоманды");
	
	Для Каждого Элемент Из ДопРеквизитыИСведения Цикл
		Подстрока = УзелДопРеквизиты.Строки.Добавить();
		Подстрока.ИмяПоляКоманды = "Ad." + XMLСтрока(Элемент.Свойство);
		Подстрока.ПредставлениеПоляКоманды = Элемент.Наименование;
		ЗаполнитьЗначенияСвойств(Подстрока, УзелДопРеквизиты, , "ИмяПоляКоманды,ПредставлениеПоляКоманды");
		
		Отбор.ИмяПоляКоманды = Подстрока.ИмяПоляКоманды;
		Для Каждого ДопСтрока Из Объект.НастройкиДоступности.НайтиСтроки(Отбор) Цикл
