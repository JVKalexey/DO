...
Процедура ЗаписатьБазовыеТипы(ЗаписьXML);
	...
	ЗаписьXML.ЗаписатьНачалоЭлемента("xs:complexType");
	ЗаписьXML.ЗаписатьАтрибут("name", "ДвоичныеДанныеОбъекта");
	ЗаписьXML.ЗаписатьНачалоЭлемента("xs:sequence");
	ЗаписатьСвойство(ЗаписьXML, "Объект", "tns:Ссылка");
	ЗаписатьСвойство(ЗаписьXML, "ДвоичныеДанные", "ns1:ValueStorage", "true");
	ЗаписьXML.ЗаписатьКонецЭлемента();
	ЗаписьXML.ЗаписатьКонецЭлемента();
...
Процедура ЗаписатьОписаниеСправочника(ЗаписьXML, ОбъектМетаданных, ДополнительныеПараметры)
	...
	Для Каждого Реквизит Из ОбъектМетаданных.Реквизиты Цикл
		...
		Если ОбъектМетаданных = Метаданные.Справочники.ВерсииФайлов Тогда
			Если Реквизит.Имя = "ПутьКФайлу" Или Реквизит.Имя = "Том" Тогда
				Необязательный = Истина;
			КонецЕсли;
		КонецЕсли;
		
		Если ОбъектМетаданных = Метаданные.Справочники.Файлы Тогда
			Если Реквизит.Имя = "ТекущаяВерсияТом" Тогда
				Необязательный = Истина;
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ОбъектМетаданных = Метаданные.Справочники.ВерсииФайлов Тогда
		ЗаписатьНеобязательноеСвойство(ЗаписьXML, "СуффиксИмениСообщенияДвоичныхДанных", "xs:string");
	КонецЕсли;
