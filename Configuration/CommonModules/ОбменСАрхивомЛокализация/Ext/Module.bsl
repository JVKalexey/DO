...
Процедура ДополнитьДокументВПакете(СвойстваКонтейнераXDTO, ВыборкаДокумент, КонтейнерДокументаXDTO, ВременнаяПапка, ТаблицаФайловДокумента) Экспорт
	...
	Пока ОбъектыДляВыгрузки.Количество() > 0 Цикл
		...
		Для Каждого МЧДСсылка Из ОбъектыДляВыгрузки Цикл
			...
			Исключение
				...
				ОбменСАрхивом.ЛогированиеОперацийОбменаСАрхивом(СобытиеЖурналаРегистрацииВыгрузкаМЧД(),
...
// Событие журнала регистрации выгрузка МЧД.
// 
// Возвращаемое значение:
//  Строка - Событие журнала регистрации выгрузка МЧД
Функция СобытиеЖурналаРегистрацииВыгрузкаМЧД()
	
	Возврат ОбменСАрхивом.СобытиеЖурналаРегистрации() + "." + НСтр("ru = 'Выгрузка МЧД'", ОбщегоНазначения.КодОсновногоЯзыка());
