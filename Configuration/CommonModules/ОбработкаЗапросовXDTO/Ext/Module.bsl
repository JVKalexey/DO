...
//
// Параметры:
//   ЗаполняемыйОбъектXDTO - ОбъектXDTO - объект XDTO список файлов которого требуется заполнить:
...
Процедура ЗаполнитьСписокСвязейОбъекта(ЗаполняемыйОбъектXDTO, ОбъектДО) Экспорт
	...
	Для Каждого КлючТипаСвязи Из ПервыйУровеньСвязей.ТипыСвязейПоПорядку Цикл
		...
		Для Каждого Строка Из ТаблицаДляСортировки Цикл
			...
			СвязьXDTO.relatedObject.comment = Строка.СведенияОбОбъекте.Комментарий;
...
//       ** Тип - Строка
//
Функция ИсточникВходящиеДанные(ВходящиеДанные, ПравилоЗагрузки, Файлы = Неопределено, Обновление = Ложь) Экспорт
...
Процедура ПроверитьАлгоритмНаЗапрещенныеВыражения(Знач АлгоритмДляПроверки) Экспорт
	...
	ЗапрещенныеСтроки.Добавить("БезопасныйРежим");
	ЗапрещенныеСтроки.Добавить("SafeMode");
	
	ЗапрещенныеСтроки.Добавить("Транзакция");
	ЗапрещенныеСтроки.Добавить("Транзакцию");
	ЗапрещенныеСтроки.Добавить("Transaction");
...
Функция СоздатьСвязьОбъекта(Сообщение)
	...
	Попытка
		...
		СвязиОбъектов.СоздатьСвязь(ИсходныйОбъектДО, СвязанныйОбъектДО, ТипСвязи);
