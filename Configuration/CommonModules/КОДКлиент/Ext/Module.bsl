...
// Выполняет действия, необходимые для настройки томов инициализированного узла.
//
// Параметры:
//	Оповещение - ОписаниеОповещения - Оповещение, которое должно быть выполнено после завершения настройки томов.
//
Процедура НачатьНастройкуТомовХраненияФайлов(Оповещение) Экспорт
	
	Контекст = Новый Структура;
	Контекст.Вставить("ОбработчикЗавершения", Оповещение);
	
	Оповещение = Новый ОписаниеОповещения(
		"НачатьНастройкуТомовХраненияФайловОбработатьРезультат", КОДСлужебныйКлиент, Контекст);
	
	ТекстВопроса =
		НСтр("ru = 'Текущий узел использует собственные тома хранения файлов, но их настройка не выполнена.
					|Установить настройки томов хранения файлов сейчас?'",
			ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(
		КодВозвратаДиалога.Да,
		Нстр("ru = 'Открыть настройки томов хранения файлов'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()));
	Кнопки.Добавить(
...
// Возвращает признак использования собственного файлового хранилища текущим узлом.
//
// Возвращаемое значение:
//	Булево - Истина, если текущий узел использует собственное файловое хранилище.
//
Функция УзелИспользуетСобственноеФайловоеХранилище() Экспорт
	
	Возврат КОДВызовСервера.УзелИспользуетСобственноеФайловоеХранилище();
