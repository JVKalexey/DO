	...
	Если ТекущиеДанные.ЭтоДокументЭДО Тогда
		...
		Иначе
			...
			Если ДанныеВизуализации.Свойство("ДвоичныеДанные") Тогда
				...
				ОбзорФайловКлиент.ЗаполнитьРеквизитыОбзора(ДанныеОбзора, Форма.ПредставлениеHTML, Форма.ТаймлистАвтопротоколHTML,
					Форма.РаспознанныйТекстHTML);
...
// Вызывается из УстановитьВидимостьСтраницОбзораФайла в форме элемента справочника ДокументыПредприятия
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения 
Процедура УстановитьВидимостьСтраницОбзораФайлаФормаЭлемента(Форма) Экспорт
	
// Локализация
	Элементы = Форма.Элементы;
	ТекущиеДанные = Форма.ТекущиеДанныеСпискаФайлов();
