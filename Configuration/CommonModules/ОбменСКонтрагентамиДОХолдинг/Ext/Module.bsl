...
Процедура УчетнаяЗаписьЭДОПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Не ЗначениеЗаполнено(УзелОбработки) Тогда
		УзелОбработки = КОДПовтИсп.ИдентификаторЦентральногоУзла();
	КонецЕсли;
...
Процедура НастройкиОтраженияДокументовВУчетеПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ЗначениеЗаполнено(ИдентификаторЭДО) Тогда
		...
		Если ДанныеУчетнойЗаписи.ИдентификаторУзлаОбмена = ОбменДаннымиДокументооборотПовтИсп.ИдентификаторТекущегоУзла() Тогда
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Если КОДПовтИсп.ЭтоЦентральныйУзел() Тогда
			Возврат;
		КонецЕсли;
...
Процедура ПриПолученииЗапросаНаОбновлениеСловаряСопоставленияНоменклатурыБЭД(Запрос) Экспорт
	...
	ИдентификаторыУзлов.Добавить(ОбменДаннымиДокументооборотПовтИсп.ИдентификаторТекущегоУзла());
	Если КОДПовтИсп.ЭтоЦентральныйУзел() Тогда
...
Функция ДанныеУчетныхЗаписейЭДОПоКОД(УчетныеЗаписиЭДО) Экспорт
	...
	ИдентификаторЦентра = КОДПовтИсп.ИдентификаторЦентральногоУзла();
	ЭтоЦентр = КОДПовтИсп.ЭтоЦентральныйУзел();
	...
	Для Каждого Элемент Из ИдентификаторыУзловКОД Цикл
		...
		ДанныеУчетнойЗаписи.ОбменВТекущемУзле = (ОбменДаннымиДокументооборотПовтИсп.ИдентификаторТекущегоУзла() = ИдентификаторУзлаКОД);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(Выборка.УзелОбработки) Тогда
			УзлыУчетныхЗаписей.Вставить(Выборка.ИдентификаторЭДО,
				КОДПовтИсп.ИдентификаторЦентральногоУзла());
		Иначе
...
Функция СводнаяСтатистикаУчетныхЗаписейЭДОПоУзлам()
	...
	Для Каждого Элемент Из ИдентификаторыУзлов Цикл
		...
		Если ИдентификаторУзла = ОбменДаннымиДокументооборотПовтИсп.ИдентификаторТекущегоУзла() Тогда
...
Функция УчетныеЗаписиСОбменомВТекущемУзле()
	...
	Для Каждого Элемент Из ИдентификаторыУзловПоУчетныхЗаписей Цикл
		...
		Если ИдентификаторУзлаКОД = ОбменДаннымиДокументооборотПовтИсп.ИдентификаторТекущегоУзла()
...
Процедура УдалитьУчетныеЗаписиДругихУзловИзСоответствияПоКлючу(Соответствие, УзлыУчетныхЗаписей, Ошибки, Сообщать)
	...
	Для Каждого Элемент Из Соответствие Цикл
		...
		Если УзлыУчетныхЗаписей[ИдентификаторЭДО] <> ОбменДаннымиДокументооборотПовтИсп.ИдентификаторТекущегоУзла() Тогда
...
Функция АдресатыДокументовЭДО(ДокументыЭДО)
	...
	ЦентральныйУзел = КОДПовтИсп.ЦентральныйУзел();
...
Функция УзлыОбработкиДокументовЭДО(ДокументыЭДО)
	...
	Для Каждого Документ Из ДокументыЭДО Цикл
		...
		Если Не ЗначениеЗаполнено(УчетнаяЗапись) Тогда
			УзлыКОДДокументов[Документ] = КОДПовтИсп.ЦентральныйУзел();
			Продолжить;
		КонецЕсли;
...
//		* ТипКлюча -см. КОДСервер.ОписаниеТиповИдентификаторовОбъектовМетаданных
//		* Объект - см. КОДСервер.ОписаниеТиповИдентификаторовОбъектовМетаданных.
//
Функция ТаблицаКлючейДанных(ТаблицаДанных)
	...
	КлючиДанных.Колонки.Добавить("ТипКлюча", КОДСервер.ОписаниеТиповИдентификаторовОбъектовМетаданных());
	КлючиДанных.Колонки.Добавить("Объект", КОДСервер.ОписаниеТиповИдентификаторовОбъектовМетаданных());
...
Функция НоваяТаблицаДанных()
	...
	ТипОбъекта = КОДСервер.ОписаниеТиповИдентификаторовОбъектовМетаданных();
