...
Функция ОтправитьВСервис(ПараметрыЗапроса, ОписанияМетодов = Неопределено)
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.БизнесСеть") Тогда
		Результат.ЕстьОшибка = Истина;
		Результат.ТекстОшибки = НСтр("ru='Ошибка аутентификации. Для аутентификации в облачном ЭДО требуется встраивание подсистемы Бизнес-сеть.'");
		Возврат Результат;
	КонецЕсли;
	
	МодульОбменБизнесСеть = ОбщегоНазначения.ОбщийМодуль("БизнесСеть");
	СтрокаАутентификации = МодульОбменБизнесСеть.СтрокаАутентификации(ПараметрыЗапроса.ИдентификаторУчетнойЗаписи,,,Ложь);
...
Функция ИдентификаторОрганизацииВСервисе(Организация) Экспорт
	...
	МодульОбменБизнесСеть = ОбщегоНазначения.ОбщийМодуль("БизнесСеть");
	Возврат МодульОбменБизнесСеть.ИдентификаторОрганизации(Организация);
