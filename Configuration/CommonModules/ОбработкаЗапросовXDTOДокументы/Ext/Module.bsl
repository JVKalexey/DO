...
Процедура ЗаполнитьУчастниковДействия(Предмет, Действие, ВидДействия, Настройка)
	...
	НаборЗаписей = РегистрыСведений.ИсторияДобавленияУчастниковДействийЧерезПравилаЗагрузкиДанныхВДО.СоздатьНаборЗаписей();
	...
	Если СтарыеУчастники.Количество() > 0
			И Не ОбщегоНазначения.КоллекцииИдентичны(НовыеУчастники.ВыгрузитьКолонку("Участник"), СтарыеУчастники.ВыгрузитьКолонку("Участник")) Тогда
	...
	Для НомерСтроки = 1 По КоличествоНовыхУчастников Цикл
		...
		СуществующиеСтроки = ДействиеОбъект.Участники.НайтиСтроки(Новый Структура("Участник", СтрокаНовыйУчастник.Участник));
		Если ЕстьФункцияУчастника Тогда
			ТребуетсяУдаление = Ложь;
			Для Каждого СуществующаяСтрока Из СуществующиеСтроки Цикл
				Если СтрокаНовыйУчастник.ФункцияУчастника = ФункцияОбработатьРезультат Тогда
					// Если для нового участника в правиле загрузки задана функция обработать результат,
					// то из старых участников нужно проверить только участника с функцией обработать результат.
					Если СуществующаяСтрока.ФункцияУчастника = ФункцияОбработатьРезультат Тогда
						ТребуетсяУдаление = Истина;
						Прервать;
					КонецЕсли;
				Иначе
					// Если для нового участника в правиле загрузки задана любая другая функция,
					// то из старых участников нужно проверить всех, кроме участника с функцией обработать результат.
					Если СуществующаяСтрока.ФункцияУчастника <> ФункцияОбработатьРезультат Тогда
						ТребуетсяУдаление = Истина;
						Прервать;
					КонецЕсли;
			...
			КонецЦикла;
		Иначе
			ТребуетсяУдаление = (СуществующиеСтроки.Количество() > 0);
		КонецЕсли;
	...
	// Заполним недостающие данные новых участников действия.
	Для Каждого СтрокаУчастник Из НовыеУчастники Цикл
		...
		Если ЕстьФункцияУчастника Тогда
			Если Не ЗначениеЗаполнено(СтрокаУчастник.ФункцияУчастника) Тогда
				СтрокаУчастник.ФункцияУчастника = ФункцияУчастникаПоУмолчанию;
			ИначеЕсли ДействияКлиентСервер.ЭтоФункцияОбработатьРезультат(СтрокаУчастник.ФункцияУчастника) И ЕстьЭтапы Тогда
				СтрокаУчастник.ИдентификаторЭтапа = ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор();
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	// Проверим корректно ли заданы новые участники действия.
	СписокОшибок = Новый Массив;
	Если ЕстьФункцияУчастника Тогда
		Для Каждого ФункцияСЕдиничнымУчастником Из ФункцииСЕдиничнымУчастником Цикл
	...
	Если ДействиеОбъект.Модифицированность() Тогда
		...
		Если ЗначениеЗаполнено(СостояниеДействия) Тогда
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Действие ""%1"" находится в состоянии ""%2"". Изменение состава участников действия запрещено.'"),
				Действие,
				СостояниеДействия)
		КонецЕсли;
