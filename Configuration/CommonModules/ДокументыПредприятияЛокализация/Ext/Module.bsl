...
Процедура УстановитьДоступностьПоСтатусуРаспознаванияДокумента(Форма) Экспорт
	...
	КолВерифицируемыхРеквизитов = Форма.ВерифицируемыеРеквизиты.Количество();
	ЭтоОбъектЭтогоУзла = ОбщегоНазначенияДокументооборот.ЭтоОбъектЭтогоУзла(ДокументСсылка);
	НеобходимаПроверкаВсехРеквизитов = Ложь;
	
	Если КолВерифицируемыхРеквизитов > 0 Тогда
		Если Форма.ВерифицируемыеРеквизиты[0].ИмяРеквизита = РаспознаваниеДокументов.ИмяРеквизитаВсе() Тогда
			НеобходимаПроверкаВсехРеквизитов = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		...
		Для Каждого Стр Из Форма.ВерифицируемыеРеквизиты Цикл   
		...
		Если ЗначениеЗаполнено(СтатусРаспознавания) и ЭтоОбъектЭтогоУзла = Ложь Тогда
			...
			ДоступностьРеквизитов = (СтатусРаспознавания = Перечисления.СтатусыРаспознаванияДокументов.Распознан 
				И ТребуетПроверки = Ложь) Или СтатусРаспознавания = Перечисления.СтатусыРаспознаванияДокументов.Прервано;
		...
		ИначеЕсли СтатусРаспознавания = Перечисления.СтатусыРаспознаванияДокументов.Распознан И ТребуетПроверки Тогда
			...
			Если НеобходимаПроверкаВсехРеквизитов = Истина Тогда
				ПредупреждающаяНадпись = НСтр("
				|	ru = 'Не удалось распознать данные документа. Необходимо проверить все реквизиты.'", 
					ОбщегоНазначения.КодОсновногоЯзыка());
			Иначе
				ПредупреждающаяНадпись = НСтр("ru = 'Документ распознан. Необходимо проверить реквизиты.'");
			КонецЕсли;
	...
	ЭлементыФормы.ГруппаДанные.Доступность = ДоступностьРеквизитов; 
	
	Если КолВерифицируемыхРеквизитов > 0 Тогда
		ВыделитьРеквизитыДляВерификации(Форма); 
	КонецЕсли;
	
	#Иначе 
		
		ВызватьИсключение НСтр("ru = 'Метод ДокументыПредприятияЛокализация.
...
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма
Процедура ВыделитьРеквизитыДляВерификации(Форма)
	...
	Для Каждого ЭлементФормы Из Форма.Элементы Цикл
		...
		Если ТипЗнч(ЭлементФормы) = Тип("ПолеФормы") Тогда
			...
			Отбор.Вставить("ИмяРеквизита", ЭлементФормы.ПутьКДанным); 
			Отбор.Вставить("ТабличнаяЧасть", "");
			
			НСтр = Форма.ВерифицируемыеРеквизиты.НайтиСтроки(Отбор);
			Если НСтр.Количество() > 0 Тогда
				Если НСтр[0].Проверен = Истина Тогда
					ЭлементФормы.ЦветФона = Новый Цвет();
					ЭлементФормы.ПодсказкаВвода = "";
				Иначе
					ЭлементФормы.ЦветФона = ЦветаСтиля.ЦветФонаПредупреждения;
					Если Не ЗначениеЗаполнено(НСтр[0].ТекущееЗначение) и ЗначениеЗаполнено(НСтр[0].РаспознанноеЗначение) Тогда
						ЭлементФормы.ПодсказкаВвода = Строка(НСтр[0].РаспознанноеЗначение); 
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		ИначеЕсли ТипЗнч(ЭлементФормы) = Тип("ТаблицаФормы") Тогда
			...
			Если Не (ЭлементФормы.Имя = "Стороны" Или ЭлементФормы.Имя = "Товары" 
				Или ЭлементФормы.Имя = "Контрагенты") Тогда
			...
			Отбор = Новый Структура("ТабличнаяЧасть", ПутьКДанным);
			НСтр = Форма.ВерифицируемыеРеквизиты.НайтиСтроки(Отбор);
			...
			ПутьКакМассив = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПутьКДанным, ".");
			
			Если ПутьКакМассив.Количество() > 1 Тогда
				ДанныеПоПути = Форма[ПутьКакМассив[0]][ПутьКакМассив[1]];
			Иначе
				ДанныеПоПути = Форма[ПутьКакМассив[0]];
			КонецЕсли;
			
			Если ДанныеПоПути.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли;
			
			ЭлементУсловногоОформления = БизнесПроцессыИЗадачиСервер.
				ЭлементУсловногоОформленияПоПредставлению(Форма.УсловноеОформление, "ВерификацияРеквизитов");
			ЭлементУсловногоОформления.Использование = Истина;
			ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветФона", 
				ЦветаСтиля.ЦветФонаПредупреждения);
			
			ЭлементУсловногоОформления.Отбор.Элементы.Очистить();
			
			Для Каждого Стр Из НСтр Цикл
				
				ОформляемоеПолеТЧ = ИмяЭлементаТЧФормыПоИмениРеквизита(ЭлементФормы, Стр.ИмяРеквизита);
				
				Если ОформляемоеПолеТЧ = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				НомерСтроки = Стр.НомерСтрокиТЧ - 1;
				ТекущееУсловноеОформление = 
						ДанныеПоПути[НомерСтроки].НеобходимаВерификация;
				Если Стр.Проверен = Истина Тогда
					ДанныеПоПути[НомерСтроки].НеобходимаВерификация = 
						СтрЗаменить(ТекущееУсловноеОформление, Стр.ИмяРеквизита, "");
				Иначе
					Если СтрНайти(ДанныеПоПути[НомерСтроки].НеобходимаВерификация, Стр.ИмяРеквизита) = 0 Тогда
						ДанныеПоПути[НомерСтроки].НеобходимаВерификация = 
							?(ПустаяСтрока(ТекущееУсловноеОформление), Стр.ИмяРеквизита, ТекущееУсловноеОформление 
								+ "," + Стр.ИмяРеквизита);
					КонецЕсли;
				КонецЕсли;

				ЭлементУсловия = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				ЭлементУсловия.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ЭлементФормы.ПутьКДанным + ".НеобходимаВерификация");
				ЭлементУсловия.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
				ЭлементУсловия.ПравоеЗначение = Стр.ИмяРеквизита;
				ЭлементУсловия.Использование = Истина;
				
				ОформляемоеПоле = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
				ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных(ОформляемоеПолеТЧ.Имя);
