...
Функция ЗапросНаПраваПоОбъекту(
	...
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.МестаХраненияДел") Тогда
		...
		ЗаменяемыеПодстроки.Вставить("&Контейнеры_ОтборСотрудников",
			?(СотрудникиОтбор = Неопределено, "ИСТИНА", "СотрудникиВКонтейнерах.Сотрудник В (&Сотрудники)"));
