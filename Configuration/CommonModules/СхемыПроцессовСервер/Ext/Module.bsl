...
//   * ЗавершенныеПутиПоиск - Массив из КлючИЗначение
//   * ЦиклыПоиск - Массив Из КлючИЗначение
...
Функция СтруктураДляПроверкиСхемы()
	...
	СтруктураДляПроверки.Вставить("ЗавершенныеПутиПоиск", Новый Массив);
	СтруктураДляПроверки.Вставить("ЦиклыПоиск", Новый Массив);
	...
	Для НомерЦикла = 0 По КоличествоЦикловОпределенияПути Цикл
		...
		Для Каждого ОбрабатываемыйПуть Из ПутиВОбработке Цикл
			...
			ИначеЕсли РезультатПродвижения = "ПутьЗавершен" Тогда
				...
				ЗавершенныйПуть = ОбщегоНазначенияДокументооборотКлиентСервер.МассивВСоответствие(ОбрабатываемыйПуть);
				СтруктураДляПроверки.ЗавершенныеПутиПоиск.Добавить(ЗавершенныйПуть);
				
			ИначеЕсли РезультатПродвижения = "ПутьИмеетЦикл" Тогда
				
				ВыделитьЦиклВПути(ОбрабатываемыйПуть);
				ЦиклВПути = ОбщегоНазначенияДокументооборотКлиентСервер.МассивВСоответствие(ОбрабатываемыйПуть);
				СтруктураДляПроверки.Циклы.Добавить(ОбрабатываемыйПуть);
				СтруктураДляПроверки.ЦиклыПоиск.Добавить(ЦиклВПути);
...
Функция ПродвинутьсяПоПутиСхемы(СтруктураДляПроверки, Путь, НовыеПутиКОбработке)
	...
	Для ИндексПоследователя = 0 По КоличествоПоследователей - 2 Цикл
		...
		Если ПутьЗавершен(СтруктураДляПроверки, НовыйПуть) Тогда
			ЗавершенныйПуть = ОбщегоНазначенияДокументооборотКлиентСервер.МассивВСоответствие(НовыйПуть);
		...
		ИначеЕсли ПутьИмеетЦикл(НовыйПуть) Тогда
			...
			ЦиклВПути = ОбщегоНазначенияДокументооборотКлиентСервер.МассивВСоответствие(НовыйПуть);
...
Процедура ПроверитьЗацикливаниеСхемы(СтруктураДляПроверки)
	...
	Для Каждого Элемент Из СтруктураДляПроверки.ЭлементыСхемы Цикл
		...
		Для Каждого Путь Из СтруктураДляПроверки.ЦиклыПоиск Цикл
			
			Если Путь[Элемент.Ключ] <> Неопределено Тогда
		...
		Для Каждого Путь Из СтруктураДляПроверки.ЗавершенныеПутиПоиск Цикл
...
Функция СостояниеЦиклаПоСвязаннымЦиклам(СтруктураДляПроверки, Элемент, ЦиклыВОбработке, СостоянияЗацикленныхПутей)
	...
	Для ИндексЦикла = 0 По СтруктураДляПроверки.Циклы.Количество() - 1 Цикл
		...
		ЦиклПути = СтруктураДляПроверки.ЦиклыПоиск[ИндексЦикла];
		
		Если ЦиклПути[Элемент] = Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Функция ЭлементСодержитсяВЗавершенномПути(СтруктураДляПроверки, Элемент)
	...
	Для Каждого Путь Из СтруктураДляПроверки.ЗавершенныеПутиПоиск Цикл
		Если Путь[Элемент] <> Неопределено Тогда
			Возврат Истина;
		КонецЕсли;	
	КонецЦикла;
...
Процедура ПроверитьКорректностьРазделенийИСлияний(СтруктураДляПроверки)
	...
	КонецЦикла;
	ЗавершенныеПутиИЦиклыПоиск = Новый Массив;
	Для Каждого ЗавершенныйПутьПоиск Из СтруктураДляПроверки.ЗавершенныеПутиПоиск Цикл
		ЗавершенныеПутиИЦиклыПоиск.Добавить(ЗавершенныйПутьПоиск);
	КонецЦикла;
	Для Каждого ЦиклПутиПоиск Из СтруктураДляПроверки.ЦиклыПоиск Цикл
		ЗавершенныеПутиИЦиклыПоиск.Добавить(ЦиклПутиПоиск);
	КонецЦикла;
	...
	Для Каждого Слияние Из СтруктураДляПроверки.Слияния Цикл
		...
		Для ИндексРазделения = 0 По Разделения.Количество() - 1 Цикл
			...
			Для Каждого Путь Из ЗавершенныеПутиИЦиклы Цикл
				...
				ПутьПоиск = ЗавершенныеПутиИЦиклыПоиск[ИндексПути];
				ИндексРазделенияВПути = ПутьПоиск[Разделение];
				ИндексСлияния = ПутьПоиск[Слияние];
				ИндексПути = ИндексПути + 1;
		...
		Иначе
			...
			Для Каждого Путь Из ЗавершенныеПутиИЦиклы Цикл
				...
				ПутьПоиск = ЗавершенныеПутиИЦиклыПоиск[ИндексПути];
				ИндексРазделенияВПути = ПутьПоиск[СвязанноеРазделение];
				ИндексСлияния = ПутьПоиск[Слияние];
				ИндексПути = ИндексПути + 1;
				...
				ИначеЕсли РазделениеВходитВТекущийПуть И Не СлияниеВходитВТекущийПуть Тогда
					...
					Если СегментПути.Количество() = 0 Тогда
						СтруктураДляПроверки.ПроверкаЗавершена = Истина;
						СтруктураДляПроверки.ЗавершениеСразуПослеРазделения = СвязанноеРазделение;
						Возврат;
					КонецЕсли;
	...
	Для ИндексРазделения = 0 По Разделения.Количество() - 1 Цикл
		...
		Для Каждого Путь Из ЗавершенныеПутиИЦиклы Цикл
			...
			ПутьПоиска = ЗавершенныеПутиИЦиклыПоиск[ИндексПути];
			ИндексРазделенияВПути = ПутьПоиска[Разделение];
			ИндексПути = ИндексПути + 1;
			РазделениеВходитВТекущийПуть = (ИндексРазделенияВПути <> Неопределено);
...
// Параметры:
//  СтруктураДляПроверки - см. СтруктураДляПроверкиСхемы
//  СегментыРазделениеСлияние - Массив Из Строка - массив последотельностей (массивов) элементов между разделением и слиянием.
