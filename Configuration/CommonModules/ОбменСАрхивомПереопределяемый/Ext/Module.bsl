...
Процедура ДобавитьДокументВПакет(ВыборкаДокумент, ПакетДокументовXDTO, ЗаписьZipФайлаПакетДокументов,
	...
	ОбменСАрхивомЛокализация.ДополнитьДокументВПакете(СвойстваКонтейнераXDTO, ВыборкаДокумент, КонтейнерДокументаXDTO, ВременнаяПапка, ТаблицаФайловДокумента);
...
Процедура ОтправитьСдаточныеОписи(КаталогВыгрузки)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			УстановитьСтатусОшибкиВыгрузкиДокумента(Выборка.Ссылка, ТекстОшибки);
		КонецПопытки;
	КонецЦикла;
	
КонецПроцедуры

Процедура ВыгрузитьСдаточнуюОписьВФайл(СдаточнаяОписьСсылка, ТекстОшибки = "")
	...
	ИмяZipФайлаВыгрузкиФинальный = ВременнаяПапка + ОбменСАрхивом.ТекущаяОтметкаВремени() + ".zip";
	...
	Попытка
		...
		ОбменСАрхивомТранспорт.ОтправитьФайл(ИмяZipФайлаВыгрузкиФинальный,	
			ОбменСАрхивомТранспорт.ПараметрыТранспортаДляСеансаОбмена());
		ОбменСАрхивом.УдалитьВременныеФайлы(ВременнаяПапка);
...
Процедура ОтправитьОтзывОписи(ОписьСсылка) Экспорт
	...
	ТекстОшибки = "";
	ВыгрузитьСдаточнуюОписьВФайл(ОписьСсылка, ТекстОшибки);
...
Процедура УстановитьСтатусОшибкиВыгрузкиДокумента(ОбъектСсылка, ТекстОшибки)
