...
// ХОЛД Начало
	ДобавитьСтандартныйОбработчик(Обработчики, "3_0_17_29");
// ХОЛД конец
...
#Область Документы_ДействиеЗадачи_ОбработатьДанныеДляПереходаНаНовуюВерсию_3_0_17_31
	...
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru = 'Основнео назначение - заполнение реквизита ""Срок обновления"" в регистрах действий задач.
...
// ХОЛД Начало
	ДобавитьСтандартныйОбработчик(Обработчики, "3_0_17_34");
	ДобавитьСтандартныйОбработчик(Обработчики, "3_0_17_35");
// ХОЛД Конец
...
Функция ВыполнитьНачальноеЗаполнениеСпискаКлючевыхОпераций() Экспорт
	...
	СписокКлючевыхОпераций.Вставить("ДействияУчастниковВыполнениеКомандыГотово", НСтр("ru = 'Действия участников обработки. Выполнение команды: Готово'", КодОсновногоЯзыка));
	СписокКлючевыхОпераций.Вставить("ДействияУчастниковОткрытиеФормыОбъекта", НСтр("ru = 'Действия участников обработки: Открытие формы объекта'", КодОсновногоЯзыка));
		...
		Для Каждого КлючИЗначение Из СписокОбъектов Цикл
			...
			Если ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(ОбъектМетаданных) Тогда
				...
				Для Каждого Элемент Из ДанныеДляОбработки Цикл
					...
					ЭлементОбъект.УзелОбработки = ОбменДаннымиДокументооборотПовтИсп.ИдентификаторТекущегоУзла();
			...
			ИначеЕсли ОбщегоНазначения.ЭтоРегистрСведений(ОбъектМетаданных) Тогда
				...
				Для Каждого НаборЗаписей Из ДанныеДляОбработки Цикл
					...
					Для Каждого Запись Из НаборЗаписей Цикл
						Запись.УзелОбработки = ОбменДаннымиДокументооборотПовтИсп.ИдентификаторТекущегоУзла();
					КонецЦикла;
...
Процедура СоздатьРабочуюГруппуРуководителиПодразделений() Экспорт
	...
	НаименованиеГруппы = Нстр("ru = 'Руководители подразделений'", МультиязычностьСервер.КодОсновногоЯзыка());
	...
	Если Результат.Следующий() Тогда
		...
		СтруктураРГ.Наименование = НСтр("ru = 'Руководители подразделений (заполняется автоматически)'",
			МультиязычностьСервер.КодОсновногоЯзыка());
	КонецЕсли;
...
// ХОЛД Начало
// Обновляет данные регистра ГраницаПереносаИсторическихДанныхКОД 
//
Процедура ПерейтиНаВерсию_3_0_17_29() Экспорт
	
	НачатьТранзакцию();
	Попытка
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ГраницаПереносаИсторическихДанныхКОД");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		Блокировка.Заблокировать();
		
		НаборЗаписей = РегистрыСведений.ГраницаПереносаИсторическихДанныхКОД.СоздатьНаборЗаписей();
		НаборЗаписей.Прочитать();
		
		Для Каждого Запись Из НаборЗаписей Цикл
			Запись.Вариант = ?(ЗначениеЗаполнено(Запись.ГраницаОкончания),
				Перечисления.ВариантыПереносаИсторическихДанныхКОД.Прямой,
				Перечисления.ВариантыПереносаИсторическихДанныхКОД.Обратный);
			Запись.РежимОтправки = ?(Запись.Вариант = Перечисления.ВариантыПереносаИсторическихДанныхКОД.Прямой,
				Перечисления.РежимыОтправкиДанныхКОД.ОтправкаПропущенных,
				Перечисления.РежимыОтправкиДанныхКОД.СтандартнаяОтправка);
		КонецЦикла;
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не удалось обновить данные регистра ГраницаПереносаИсторическихДанныхКОД по причине:
						|%1'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		ЗаписьЖурналаРегистрации(
			ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Ошибка,
			Метаданные.РегистрыСведений.ГраницаПереносаИсторическихДанныхКОД,,
			ТекстСообщения);
...
// ХОЛД Начало
// Заполняет реквизит УзелОбработки в справочнике ТомаХраненияФайлов.
//
Процедура ПерейтиНаВерсию_3_0_17_34() Экспорт
	
	НачатьТранзакцию();
	Попытка
		ИдентификаторЦентральногоУзла = КОДПовтИсп.ИдентификаторЦентральногоУзла();
		...
		Запрос = Новый Запрос(
			...
			|ИЗ
			|	Справочник.ТомаХраненияФайлов КАК Таблица
			|ГДЕ
			|	Таблица.УзелОбработки = &ПустойИдентификатор");
		...
		Пока Выборка.Следующий() Цикл
			...
			Объект.УзелОбработки = ИдентификаторЦентральногоУзла;
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не удалось обновить данные справочника ТомаХраненияФайлов по причине:
						|%1'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	РаспознаваниеДокументов.ОбновитьПолномочия();
	
КонецПроцедуры

// Заполняет реквизит УзелОбработки в справочнике ПравилаРазмещенияФайловВТомах.
//
Процедура ПерейтиНаВерсию_3_0_17_35() Экспорт
	...
	Попытка
		...
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("Справочник.ПравилаРазмещенияФайловВТомах");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		Блокировка.Заблокировать();
		...
		Запрос = Новый Запрос(
			...
			|	Таблица.Ссылка КАК Ссылка
			|ИЗ
			|	Справочник.ПравилаРазмещенияФайловВТомах КАК Таблица
			|ГДЕ
			|	Таблица.УзелОбработки = &ПустойИдентификатор");
		Запрос.УстановитьПараметр(
			"ПустойИдентификатор", ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор());
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не удалось обновить данные справочника ПравилаРазмещенияФайловВТомах по причине:
						|%1'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		ЗаписьЖурналаРегистрации(
			ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Ошибка,
			Метаданные.Справочники.ПравилаРазмещенияФайловВТомах,
			Выборка.Ссылка,
			ТекстСообщения);
