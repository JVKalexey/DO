...
Процедура ЗафиксироватьОтметкуВремениРегистра(Источник, Замещение) Экспорт
	...
	ОписаниеОтметкиВремени = ОписаниеОтметкиВремени(Источник); // см. ОписаниеСвойствОтметкиВремени
	...
	Если Не ОтборНабораПолный И (Замещение Или ЭтоУдалениеНабора)
		...
		КлючиОтборыЗамещаемыхЗаписей = КлючиОтборыЗамещаемыхЗаписей(
			ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(ОписаниеОтметкиВремени.Объект),
			Источник.Отбор,
			ОписаниеОтметкиВремени.КлючиИсточника);
		...
		Для Каждого ЗамещаемаяЗапись Из КлючиОтборыЗамещаемыхЗаписей Цикл
			...
			Запись.ТипКлюча = ЗамещаемаяЗапись.ТипКлюча;
	...
	Если ЗначениеЗаполнено(ОписаниеОтметкиВремени.КлючиИсточника) Тогда
		Для Каждого Строка Из ОписаниеОтметкиВремени.КлючиИсточника Цикл
			Если Строка.ИдентификаторКлюча <> ОписаниеОтметкиВремени.ИдентификаторКлюча
					Или Строка.ТипКлюча <> ОписаниеОтметкиВремени.ТипКлюча Тогда
				
				Запись.ИдентификаторКлюча = Строка.ИдентификаторКлюча;
				Запись.ТипКлюча = Строка.ТипКлюча;
				Запись.ЗначенияКлюча = Неопределено;
...
Функция ОписаниеОтметкиВремени(Источник)
	...
	Если ОписаниеКлюча.ВидКлюча = Перечисления.ВидыКлючейОтметокВремени.КлючНабораПервоеСсылочноеИзмерение Тогда
	...
	ИначеЕсли ОписаниеКлюча.ВидКлюча = Перечисления.ВидыКлючейОтметокВремени.КлючНабораИдентификаторНабораЗаписей Тогда
		...
		Для Каждого ЗначениеПоля Из ЗначенияПоля Цикл
			КлючИсточника = ОписаниеОтметкиВремени.КлючиИсточника.Добавить();
			КлючИсточника.ИдентификаторКлюча = ЗначениеПоля["ИдентификаторОтметкиВремени"]; // УникальныйИдентификатор;
...
//	КлючиЗамещающегоНабора - ТаблицаЗначений:
//		* ИдентификаторКлюча - УникальныйИдентификатор - Идентификатор ключа.
//		* ТипКлюча - СправочникСсылка.ИдентификаторыОбъектовМетаданных - Тип ключа.
//					- СправочникСсылка.ИдентификаторыОбъектовРасширений -
//
// Возвращаемое значение:
//	Массив Из Структура:
...
//
Функция КлючиОтборыЗамещаемыхЗаписей(ОбъектМетаданных, Отбор, КлючиЗамещающегоНабора)
	...
	Пока ЗамещаемыеЗаписи.Следующий() Цикл
		...
		ОписаниеЗамещаемогоКлюча = Новый Структура;
		ОписаниеЗамещаемогоКлюча.Вставить(
			"ИдентификаторКлюча", ОтметкиВремени.ИдентификаторНабораЗаписей(КлючевойНабор));
		ОписаниеЗамещаемогоКлюча.Вставить("ТипКлюча", Неопределено);
		
		Если ЗначениеЗаполнено(КлючиЗамещающегоНабора)
				И КлючиЗамещающегоНабора.НайтиСтроки(ОписаниеЗамещаемогоКлюча).Количество() > 0 Тогда
			Продолжить;
		КонецЕсли;
		
		ОписаниеЗамещаемогоКлюча.Вставить(
			"ЗначенияОтбораКлюча", Новый ХранилищеЗначения(КлючевойНабор.Отбор, Новый СжатиеДанных(9)));
		
		КлючиЗаписейЗамещаемогоОтбора.Добавить(ОписаниеЗамещаемогоКлюча);
	КонецЦикла;
