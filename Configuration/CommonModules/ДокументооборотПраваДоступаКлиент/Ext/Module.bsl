...
// Для полноправного пользователя. Проверяет, есть ли некорректные роли из БСП, которые приведут к ошибке, если
// редактировать полномочия. Если найдены, будет выдано сообщение администратору.
Процедура ЕстьЛиНекорректныеРоли() Экспорт
	
	Если Не ПользователиКлиент.ЭтоПолноправныйПользователь() Тогда
		Возврат;
	КонецЕсли;
	
	Текст = ДокументооборотПраваДоступаВызовСервера.НекорректныеРолиСотрудников();
	Если Текст = "" Тогда
		Возврат;
	КонецЕсли;
