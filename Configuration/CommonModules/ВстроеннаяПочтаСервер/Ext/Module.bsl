...
Функция ВычислитьПриоритетноеПредставление(МассивКонтакты) Экспорт
	...
	Для Каждого Контакт Из МассивКонтакты Цикл
		...
		Если ТипЗнч(Контакт) = Тип("СправочникСсылка.Сотрудники") Тогда
			РеквизитыКонтакта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контакт, "ПредставлениеВПереписке, Действует");
			Если РеквизитыКонтакта.Действует = Истина Тогда
...
Процедура ПолучитьВсеПисьма(СообщенияОбОшибках, ПараметрыЛогирования, МассивУчетныхЗаписей = Неопределено)
	...
	Иначе	
		...
		ПараметрыОтбора.Вставить("ВариантИспользования", Перечисления.ВариантыИспользованияПочты.Встроенная);
		ПараметрыОтбора.Вставить("УзелОбработки", ОбменДаннымиДокументооборотПовтИсп.ИдентификаторТекущегоУзла());
		УчетныеЗаписи = Почта.ПолучитьУчетныеЗаписиЭлектроннойПочты(ПараметрыОтбора);
	КонецЕсли;		
