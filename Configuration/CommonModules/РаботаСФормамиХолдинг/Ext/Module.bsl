...
Процедура ИзменитьФормуНастроекПрограммы(Форма) Экспорт
	...
	Если Форма.ИмяФормы = "Обработка.НастройкаПрограммы.Форма.ОсновнаяФорма" Тогда
		...
		Если КОДПовтИсп.ЭтоПодчиненныйУзел() Тогда
	...
	Если Форма.ИмяФормы = "Справочник.ТомаХраненияФайлов.Форма.ФормаЭлемента"
		Или Форма.ИмяФормы = "Справочник.ТомаХраненияФайлов.Форма.ФормаСписка"
		Или Форма.ИмяФормы = "Справочник.ТомаХраненияФайлов.Форма.ФормаГруппы"
		...
		Если Не КОДПовтИсп.УзелИспользуетСобственноеФайловоеХранилище() Тогда
			...
			Если Форма.ИмяФормы = "Справочник.ТомаХраненияФайлов.Форма.ФормаСписка" Тогда
				Форма.Элементы.СписокПереместитьВверх.Доступность = Ложь;
				Форма.Элементы.СписокКонтекстноеМенюПереместитьВверх.Доступность = Ложь;
				Форма.Элементы.СписокПереместитьВниз.Доступность = Ложь;
				Форма.Элементы.СписокКонтекстноеМенюПереместитьВниз.Доступность = Ложь;
				Форма.Элементы.ФормаПереносФайловВТома.Доступность = Ложь;
			КонецЕсли;
			
			Если Форма.ИмяФормы =
					"Справочник.ПравилаРазмещенияФайловВТомах.Форма.НастройкаПравилРазмещенияФайловВТомах" Тогда
				
				Форма.Элементы.ПравилаПереместитьВверх.Доступность = Ложь;
				Форма.Элементы.ПравилаКонтекстноеМенюПереместитьВверх.Доступность = Ложь;
				Форма.Элементы.ПравилаПереместитьВниз.Доступность = Ложь;
				Форма.Элементы.ПравилаКонтекстноеМенюПереместитьВниз.Доступность = Ложь;
				Форма.Элементы.ФормаОК.Доступность = Ложь;
			КонецЕсли;
	...
	Если Форма.ИмяФормы = "Справочник.ШаблоныИменФайлов.Форма.ФормаЭлемента"
		Или Форма.ИмяФормы = "Обработка.НастройкаПрограммы.Форма.НастройкаСканирования" Тогда
...
Процедура ИнформационнаяНадписьЦентрализованныеНастройки(Форма)
	...
	ИнформационнаяНадпись = РаботаСФормами.ДобавитьНадпись(
		...
		ГруппаЦентрализованныеНастройки,
		НСтр("ru = 'Некоторые настройки программы могут быть изменены только в центральном узле'"));
	ИнформационнаяНадпись.ЦветФона = ЦветаСтиля.ОбъектПомеченНаУдаление;
	ИнформационнаяНадпись.АвтоМаксимальнаяШирина = Ложь;
	ИнформационнаяНадпись.РастягиватьПоГоризонтали = Истина;
