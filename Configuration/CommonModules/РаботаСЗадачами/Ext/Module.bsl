...
Функция ОпределитьФактическихИсполнителей(ДействияЗадач, ВыполнитьОтИмени, ВыбранныйФактическийИсполнитель, РеквизитыДействийЗадач = Неопределено, РазрешитьПустогоОтметил = Ложь) Экспорт
	...
	АдресатыДействийЗадач = АдресатыДействийЗадач(ДействияЗадач);
...
//
Функция ДанныеСоисполнителей(Задача, ДействиеЗадачи, КлючНазначения, КешДанных = Неопределено, ПроверятьПрава = Истина) Экспорт
...
Функция ДанныеДереваПриложений(Задача, ДействиеЗадачи, КешДанных = Неопределено) Экспорт
	...
	ДанныеСоисполнителей = ДанныеСоисполнителей(Задача, ДействиеЗадачи, "ЕстьСоисполнители", Неопределено, Ложь);
	...
	Для Каждого СтрокаПриложения Из ДанныеДереваПриложений Цикл // См. РаботаСЗадачами.НоваяСтрокаДереваПриложений
		...
		ЕстьПраво = ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(СтрокаПриложения.Приложение).Чтение;
		...
		Если ДелопроизводствоКлиентСервер.ЭтоДокумент(СтрокаПриложения.Приложение) Тогда
			...
			РаботаСЗадачамиЛокализация.ДополнитьСтрокуПриложенияДереваПриложений(Задача, ДействиеЗадачи,
...
Функция ДанныеДереваПодзадач(Задача, ДействиеЗадачи) Экспорт
	...
	Пока ДействияТекущегоУровня.Количество() > 0 Или ЭтоПервыйУровень Цикл
		...
		Для Каждого СтрокаДанныеДействия Из ДанныеДействийПодзадач Цикл
			СтрокаДанныеДействия.ПредставлениеУчастника = Строка(СтрокаДанныеДействия.Участник);
		КонецЦикла;
...
Функция ДоступнаОтменаВыполнения(ДействиеЗадачи, КешДанных = Неопределено)
	...
	Если ТипЗнч(ДействиеЗадачи) = Тип("СтрокаТаблицыЗначений") Тогда
		РеквизитыДействия = ДействиеЗадачи;
	ИначеЕсли КэшОбластиПросмотра.Заполнен И КэшОбластиПросмотра.ДействиеЗадачи = ДействиеЗадачи Тогда
		РеквизитыДействия = КэшОбластиПросмотра.РеквизитыДействияЗадачи;
	Иначе
		РеквизитыДействия = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ДействиеЗадачи,
...
Функция КоличествоПодзадач(Задача, ДействиеЗадачи)
	...
	Для Каждого СтрокаДействияЗадачи Из АктивныеДействияПоЗадачам Цикл
		...
		РеквизитыПодзадачи = РеквизитыПодзадач[СтрокаДействияЗадачи.Задача];
		Если РеквизитыПодзадачи = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаУчастника = РеквизитыПодзадачи.Участники.Найти(
			СтрокаДействияЗадачи.ДействиеЗадачи,
			"ДействиеУчастника");
		Если СтрокаУчастника = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ЭтоОжидающаяПроверкиЗадача = ЭтоОжидающиеПроверкиЗадачи[СтрокаДействияЗадачи.Задача];
		Если Не ЭтоОжидающаяПроверкиЗадача
			И РаботаСЗадачамиПовтИсп.ЭтоПроверяющий(СтрокаУчастника.ВидУчастника) Тогда
			Продолжить;
		КонецЕсли;
...
// 
Функция АдресатыДействийЗадач(ДействияЗадач)
