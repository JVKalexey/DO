...
Функция ПраваСотрудникаПоОбъекту(ОбъектДоступа, Сотрудник) Экспорт
	...
	УстановитьОтключениеБезопасногоРежима(Истина); // Может быть вызвано из "Алгоритмов".
...
//
// Параметры:
//  ОбъектыДоступа - Массив Из ЛюбаяСсылка - массив ссылок на объекты, чьи права нужно получить
//  УчитыватьНеограниченныеПрава - Булево - признак включения в результат пользователей с неограниченными правами на таблицу.
//  СотрудникиОтбор - Массив Из СправочникСсылка.Сотрудники - массив сотрудников для отбора.
//					- Неопределено - Если передано Неопределено, то по всем сотрудникам.
// 
// Возвращаемое значение:
...
Функция ПраваСотрудниковПоОднотипнымОбъектам(
	...
	КлючеваяОперация = "ПроверкаНаличияПравДоступа";
	...
	Если ОбщегоНазначенияДокументооборотПовтИсп.ЕстьМетодМенеджера(
		...
		ТаблицаПрав = Менеджер.ПраваСотрудниковПоОбъектам(ОбъектыДоступа, СотрудникиОтбор);
	...
	Если ЗаменяемыеПодстроки.Получить("&ПраваПоДескрипторам_ОтборПравообладателей") = Неопределено Тогда
		ЗаменяемыеПодстроки.Вставить("&ПраваПоДескрипторам_ОтборПравообладателей", 
			?(СотрудникиОтбор = Неопределено, "ИСТИНА",
				"ПраваПоДескрипторамДоступаОбъектов.Сотрудник В (&Сотрудники)"));
	КонецЕсли;
	
	Если ЗаменяемыеПодстроки.Получить("&СоставСубъектов_ОтборПравообладателей") = Неопределено Тогда
		ЗаменяемыеПодстроки.Вставить("&СоставСубъектов_ОтборПравообладателей",
			?(СотрудникиОтбор = Неопределено, "ИСТИНА",
				"СоставСубъектов.Сотрудник В (&Сотрудники)"));
	КонецЕсли;
	...
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.Задача") Тогда
		...
		Запрос.Текст =
			...
			|		И &ОтборПоЗамещающемуСотруднику
			|) КАК ВложенныйЗапрос
			|СГРУППИРОВАТЬ ПО
			|	ВложенныйЗапрос.ОбъектДоступа,
			|	ВложенныйЗапрос.Сотрудник";
		...
		ЗаменяемыеПодстроки.Вставить("&ОтборПоЗамещающемуСотруднику",
			?(СотрудникиОтбор = Неопределено, "ИСТИНА", "ЗамещающиеИПомощники.Замещающий В (&Сотрудники)"));
	...
	ИначеЕсли ДокументооборотПраваДоступаЛокализация.ЗапросПравОднотипныхОбъектовПодлежитЛокализации(ПервыйОбъектДоступа) Тогда
		
		ДокументооборотПраваДоступаЛокализация.ЗаполнитьЗапросПравОднотипныхОбъектов(ПервыйОбъектДоступа,
			Запрос, МетаданныеОбъекта);
...
Функция ОпределитьДескрипторыОбъекта(ОбъектДоступа, ОпределятьЗависимыеПрава = Истина, ПротоколРасчетаПрав = Неопределено) Экспорт
	...
	ИначеЕсли ДокументооборотПраваДоступаЛокализация.ОпределениеДескрипторовОбъектаПодлежитЛокализации(ОбъектДоступа) Тогда
		ДокументооборотПраваДоступаЛокализация.ОпределитьДескрипторыОбъекта(ОбъектДоступа, ТаблицаДескрипторов, ОпределятьЗависимыеПрава, ПротоколРасчетаПрав);
	Иначе
...
Процедура РасширитьТаблицуПравНеограниченнымиПравами(
	...
	Если СотрудникиОтбор = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Контейнеры_ОтборПравообладателей", "ИСТИНА");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
			"&Контейнеры_ОтборПравообладателей", "СотрудникиВКонтейнерах.Сотрудник В (&Сотрудники)");
	КонецЕсли;
...
Процедура ДополнитьТекстЗапросаПоПравам(ТекстЗапроса, ДляДескрипторовРегистров = Ложь) Экспорт
	...
	ДополнениеТекстаЗапроса = "
		...
		|				И &ИмяПоляИОМ = СоставСубъектовПравДоступа.ОбъектМетаданных
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
