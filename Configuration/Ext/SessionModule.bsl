...
// ХОЛД Начало	
	Если ИменаПараметровСеанса = Неопределено
			Или ИменаПараметровСеанса.Найти("ЭтоСеансКОД") <> Неопределено Тогда
		ПараметрыСеанса.ЭтоСеансКОД = Ложь;
	КонецЕсли;
	
	Если ИменаПараметровСеанса = Неопределено
			Или ИменаПараметровСеанса.Найти("УзелОтправительКОД") <> Неопределено Тогда
		ПараметрыСеанса.УзелОтправительКОД = Справочники.УзлыКОД.ПустаяСсылка();
	КонецЕсли;
	
	Если ИменаПараметровСеанса = Неопределено
			Или ИменаПараметровСеанса.Найти("ЭтоЦентральныйУзелКОД") <> Неопределено Тогда
		
		ПараметрыСеанса.ЭтоЦентральныйУзелКОД = Истина;
	КонецЕсли;
	
	Если ИменаПараметровСеанса = Неопределено
			Или ИменаПараметровСеанса.Найти("АдресХранилищаДанныхОтложеннойЗаписи") <> Неопределено Тогда
		ПараметрыСеанса.АдресХранилищаДанныхОтложеннойЗаписи = "";
	КонецЕсли;	
