...
Процедура ДобавитьФайл(ВерсияФайлаСсылка) Экспорт
...
Процедура СброситьОшибку(ВерсияФайлаСсылка) Экспорт
	...
	Попытка
		...
		НаборЗаписей[0].ДатаПередачиНаРаспознавание = УниверсальноеВремя(ТекущаяДата(), ЧасовойПояс());
...
// Фиксирует ошибку баланса для всех распознаваемых файлов
Процедура ЗафиксироватьОшибкиБаланса() Экспорт
	...
	ОписаниеОшибки = НСтр("ru = 'Ошибка баланса. Баланс нулевой или истек срок действия тарифа'");
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		
		ЗафиксироватьОшибкуПриРаспознавании(Выборка.Файл, ОписаниеОшибки);
